...
const myRowsRef: Ref<{ time: string; date: string; name: string }[]> = ref(getList());
...
function shortList(PageCounter: number){
  transferData.value = [];
  let myRowsReversed: { time: string; date: string; name: string }[] = [ ...myRows,];
  myRowsReversed.reverse();

    for(let CountTen = PageCounter * 10; CountTen < PageCounter * 10 + 10; CountTen++){
...
<template>
    <div class="CenterBlock">
        ...
        <button v-for="page in PagesArray" :key="page" @click="setCurrentPage(page)" :class="page == currentPage ? 'red' : 'green'"> <!-- ? inline-if, : inline-else  -->
            {{ page + 1 }}      
        </button>
...
body{
  animation: background-pan 9s linear infinite;
